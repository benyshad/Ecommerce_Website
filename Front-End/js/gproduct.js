...
//display products
class UI {
  static getGoogleProduct() {
    const queryString = window.location.search;
    const urlParams = new URLSearchParams(queryString);
    const productId = urlParams.get("productId");
    let googleItem = { ...Storage.getProductById(productId) };
    let clickedItem = googleItem;
    let qtySelectorDom = document.querySelector("#qty");
    let stockDom = document.querySelector("#stock");
    
    carouselActiveImg.innerHTML = `<img src="${googleItem.imgLinks[0]}" class="d-block w-100 ms-4 ps-4" style="max-height:600px; max-width:600px;" alt="...">`;
    for (let i = 1; i < googleItem.imgCount; i++) {
      ...
      carouselImg.innerHTML += `<div class="carousel-item">
            <img src="${googleItem.imgLinks[i]}" class="d-block w-100ms-4 ps-4" style="max-height:600px; max-width:600px;" alt="...">
          </div>`;
    }
    title.innerHTML = googleItem.title;
    price.innerHTML = `USD $${googleItem.price}`;
    let googleItemID = parseInt(googleItem.id);
    fetch(`./products/${googleItemID}/description.txt`)
      .then((response) => response.text())
      .then((data) => {
        // Do something with your data
        description.innerHTML = data;
      });
    //localStorage.setItem("GoogleItem", JSON.stringify(googleItem));
  ...
  addToCart() {
    ...
    addToCart.addEventListener("click", (Event) => {
      ...
      const urlParams = new URLSearchParams(queryString);
      const productId = urlParams.get("productId");
      let clickedItem = { ...Storage.getProductById(productId) };
  ...
  static getCustomerReview() {
    ...
    fetch(`./products/${googleItemID}/reviews.json`)
